...
.. note:: **TL;DR**
   - A production Jenkins pipeline architecture for **embedded OS development** where an **upstream Yocto build pipeline** produces artifacts and a **downstream Labgrid testing pipeline** securely retrieves them using ``copyArtifacts`` with ``specific()`` selector — enforcing immutability, traceability, and concurrency safety.
   - Uses **Amarula Solutions' Jenkins shared libraries** (``Build``, ``Changelog``, ``Ui``) with kas-container builds, Mend SCA integration, and hardware-in-the-loop testing via Labgrid.
...
How does the upstream build pipeline produce artifacts?
-------------------------------------------------------
...
How does the downstream testing pipeline consume artifacts?
-----------------------------------------------------------
...
What are the benefits of specific artifact copying?
---------------------------------------------------
...
.. tip::
   Need a tightly integrated CI/CD pipeline with hardware-in-the-loop testing?
   Amarula Solutions designs Jenkins-based build and test automation with
   Labgrid integration for embedded Linux and Android projects.
