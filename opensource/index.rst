Open Source Projects
====================

.. note:: **TL;DR**
   - Amarula Solutions' **open source project hub** — featuring our own products (CuteKeyboard, Flutekeyboard, ChatGPT Gerrit plugin, ldap-passwd-webui, DynDesign, libcppconnman, meta-mend) and third-party integration guides for **U-Boot, OP-TEE, Zephyr, Buildroot, and Linux multimedia/DRM**.
   - Visit the **Open Source Contributions page** for our maintainer roles, monthly upstream reports, and contribution statistics.

.. _contents: Table of contents

.. toctree::
   :maxdepth: 1
   :caption: Our Contributions

   contributions

.. toctree::
   :maxdepth: 1
   :titlesonly:
   :caption: Our Products

   products/dyndesign/index
   products/chatgpt-gerrit
   products/cutekeyboard
   products/flutekeyboard
   products/meta-mend
   products/libcppconnman
   products/git-collect-plugin
   products/ldap-passwd-webui

.. toctree::
   :maxdepth: 1
   :caption: Third-Party Integration Guides

   thirdparty/index

.. tip::
   Interested in our open source work or want to contribute? Amarula Solutions
   actively funds upstream development and welcomes collaboration on embedded
   Linux, U-Boot, Buildroot, Yocto, Zephyr, and Jenkins tooling.
   `Contact our open source team <https://www.amarulasolutions.com/contact/>`_
